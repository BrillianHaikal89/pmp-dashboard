...
import {
  IndikatorPrioritasSingle,
  IndikatorPrioritasBanding,
} from "./components/prioritas";
...
function DashboardContent() {
      ...
      <div className="flex h-screen items-center justify-center bg-slate-50">
        <div className="text-center">
          ...
          <p className="text-slate-500 mb-4">
            Maaf, wilayah yang Anda cari tidak tersedia.
          </p>
          <a
            href="/pilih-wilayah"
  ...
  const renderContent = () => {
    if (isLoading)
      return (
        <LoadingScreen
          msg={`Memuat data ${tahun === "banding" ? "2024 & 2025" : "tahun " + tahun}...`}
        />
      );
    ...
    switch (active) {
      case "ringkasan":
        if (tahun === "banding")
          return data24 && data25 ? (
            <RingkasanBanding d24={data24} d25={data25} />
          ) : (
            <LoadingScreen />
          );
        return activeData ? (
          <RingkasanSingle data={activeData} tahun={tahun} />
        ) : (
          <LoadingScreen />
        );
      ...
      case "kabkot":
        ...
        return activeData ? (
          <KabkotSingle data={activeData.kabkot ?? []} tahun={tahun} />
        ) : (
          <LoadingScreen />
        );
      ...
      case "satdik":
        ...
        return activeData ? (
          <SatdikSingle data={activeData.satdik ?? []} tahun={tahun} />
        ) : (
          <LoadingScreen />
        );
      ...
      case "akar":
        ...
        return activeData ? (
          <AkarPage data={activeData.akar_masalah ?? []} tahun={tahun} />
        ) : (
          <LoadingScreen />
        );
      ...
      case "pemda":
        if (tahun === "banding")
          return data24 && data25 ? (
            <PemdaBanding d24={data24} d25={data25} />
          ) : (
            <LoadingScreen />
          );
        return activeData ? (
          <PemdaTable data={activeData} tahun={tahun} />
        ) : (
          <LoadingScreen />
        );
        ...
        <header className="bg-white border-b border-slate-100 px-4 sm:px-5 py-3 flex items-center gap-3 shadow-sm flex-shrink-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-slate-400 hidden sm:block">
              Rapor Pendidikan {wilayahInfo.displayName}
            </p>
          </div>
          ...
          <div className="hidden sm:flex items-center gap-2 flex-shrink-0">
            {data24 && (
              <span className="flex items-center gap-1 px-2.5 py-1 bg-blue-50 text-blue-700 rounded-full text-xs font-bold border border-blue-100">
                <span className="text-blue-400">2024</span> {data24.spm_value}
              </span>
            )}
            {data25 && (
              <span className="flex items-center gap-1 px-2.5 py-1 bg-violet-50 text-violet-700 rounded-full text-xs font-bold border border-violet-100">
                <span className="text-violet-400">2025</span> {data25.spm_value}
              </span>
            )}
          </div>
        </header>
