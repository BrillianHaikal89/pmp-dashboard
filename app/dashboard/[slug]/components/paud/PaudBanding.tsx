...
export function PaudBanding({ d24, d25 }: { d24: PaudRow[]; d25: PaudRow[] }) {
  ...
  const improved = merged.filter(r => getLabelScore(r.r25?.label_perencanaan ?? "") > getLabelScore(r.label_perencanaan ?? "")).length;
  const declined = merged.filter(r => getLabelScore(r.r25?.label_perencanaan ?? "") < getLabelScore(r.label_perencanaan ?? "")).length;
  const same = merged.filter(r => r.r25 && getLabelScore(r.r25.label_perencanaan ?? "") === getLabelScore(r.label_perencanaan ?? "")).length;
  ...
  const cmpChart = inds.map((key, idx) => ({ name: indNames[idx], baik24: d24.filter(d => (d as any)[key] === "Baik").length, baik25: d25.filter(d => (d as any)[key] === "Baik").length }));
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="bg-slate-50 border-b border-slate-100"><th className="text-left py-3 px-3 text-xs font-bold text-slate-500">NPSN</th><th className="text-left py-3 px-3 text-xs font-bold text-slate-500">Nama</th><th className="text-left py-3 px-3 text-xs font-bold text-slate-500">Kecamatan</th><th className="text-left py-3 px-3 text-xs font-bold text-blue-500">Perencanaan 2024</th><th className="text-left py-3 px-3 text-xs font-bold text-violet-500">Perencanaan 2025</th><th className="text-left py-3 px-3 text-xs font-bold text-blue-500">Sarana 2024</th><th className="text-left py-3 px-3 text-xs font-bold text-violet-500">Sarana 2025</th><th className="text-left py-3 px-3 text-xs font-bold text-rose-500">Tren</th></tr></thead>
            <tbody>{paged.map((row, i) => (<tr key={i} className={`border-b border-slate-50 hover:bg-slate-50 ${!row.r25 ? "opacity-50" : ""}`}><td className="py-2.5 px-3 font-mono text-xs text-slate-400">{row.npsn}</td><td className="py-2.5 px-3 text-xs font-semibold text-slate-800 max-w-[150px] truncate">{row.nama}</td><td className="py-2.5 px-3 text-xs text-slate-500">{row.kecamatan}</td><td className="py-2.5 px-3"><Badge label={row.label_perencanaan ?? ""} /></td><td className="py-2.5 px-3">{row.r25 ? <Badge label={row.r25.label_perencanaan ?? ""} /> : <span className="text-xs text-slate-300">—</span>}</td><td className="py-2.5 px-3"><Badge label={row.label_sarana ?? ""} /></td><td className="py-2.5 px-3">{row.r25 ? <Badge label={row.r25.label_sarana ?? ""} /> : <span className="text-xs text-slate-300">—</span>}</td><td className="py-2.5 px-3">{row.r25 ? <TrendBadge val24={row.label_perencanaan} val25={row.r25.label_perencanaan} /> : null}</td></tr>))}</tbody>
          </table>
        </div>
